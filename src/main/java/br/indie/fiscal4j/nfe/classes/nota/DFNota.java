package br.indie.fiscal4j.nfe.classes.nota;

import br.indie.fiscal4j.common.DFBase;
import br.indie.fiscal4j.nfe.classes.nota.assinatura.DFSignature;
import org.simpleframework.xml.Element;
import org.simpleframework.xml.Namespace;
import org.simpleframework.xml.Root;

@Root(name = "NFe")
@Namespace(reference = "http://www.portalfiscal.inf.br/nfe")
public class DFNota extends DFBase {

    private long identificadorLocal;

    @Element(name = "infNFe")
    private DFNotaInfo info;

    @Element(name = "infNFeSupl", required = false)
    private DFNotaInfoSuplementar infoSuplementar;

    @Element(name = "Signature", required = false)
    private DFSignature assinatura;

    public void setInfo(final DFNotaInfo info) {
        this.info = info;
    }

    public DFNotaInfo getInfo() {
        return this.info;
    }

    public DFNotaInfoSuplementar getInfoSuplementar() {
        return infoSuplementar;
    }

    public void setInfoSuplementar(DFNotaInfoSuplementar infoSuplementar) {
        this.infoSuplementar = infoSuplementar;
    }

    /**
     * Utilizado para identificacao interna do sistema
     *
     * @param identificadorLocal Identificador.
     */
    public void setIdentificadorLocal(final long identificadorLocal) {
        this.identificadorLocal = identificadorLocal;
    }

    public long getIdentificadorLocal() {
        return this.identificadorLocal;
    }

    public void setAssinatura(final DFSignature assinatura) {
        this.assinatura = assinatura;
    }

    public DFSignature getAssinatura() {
        return this.assinatura;
    }
}